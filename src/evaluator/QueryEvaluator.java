...
public class QueryEvaluator extends PositionalInvertedIndex {
    ...
    private List<Document> intersect(List<Document> postOne, List<Document> postTwo) {
        ...
        while (indexOne < postOne.size() && indexOne < postTwo.size() && indexTwo < postTwo.size()) {
            ...
            if (docIdOne == docIdTwo) {
                ...
                result.add(postTwo.get(indexTwo)); // result.add(postTwo.get(indexOne));
                indexOne++;
                indexTwo++;
    ...
    private WeightedPost weighTerm(Document term) {
        ...
        //weighs term
        //double termWeigh = (1 + Math.log10(termFreq)) * Math.log10(positionalIndex.size() / docFreqTerm);
     ...
     */
    private void saveQueryResultToFile(List<WeightedDocument> result, String query) {
        try (FileWriter writer = new FileWriter("QueryResult.txt", true)) {
            writer.write(query + "\n");
            //the query returned a document set
            if (result != null && result.size() > 0) {
                writer.write("documents and relevance scores:\n");
                for (WeightedDocument document : result) {
                    writer.write("Doc id:" + document.getDocId() + "\tscore: " + document.getWeight() + "\n");
                }
                writer.write("\n");
            } else {
    ...
    private void evaluateQuery(String query) {
        ...
        //merge regular and proximity ranked terms
        ...
        //rank document
        List<WeightedDocument> ranked = rankDocuments(regQueryResult);

        //save result
        saveQueryResultToFile(ranked, query);
        //print result
        /*for (WeightedDocument doc : ranked) {
            System.out.printf("%d %s %.2f", doc.getDocId(),":", doc.getWeight());
            System.out.println();
        }*/
    ...
    public static void main(String[] args) {
        QueryEvaluator queryEvaluator = new QueryEvaluator(args[0]);
        queryEvaluator.run();
